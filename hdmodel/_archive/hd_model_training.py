...
# Pre-computed once; used for W construction, optimizer, and plot function
...
def plot_mexican_hat(J1=4.0, J0=1.5):
    ...
    ax.fill_between(theta_deg, W, 0, where=(W > 0), interpolate=True,
...
im = ax1.imshow(
    ...
    cmap='magma', vmin=0, vmax=np.max(tuning_heatmap),
)
ax1.set_title("Network Tuning Curve Heatmap\n(Steady-State Firing Rates)", fontsize=14)
ax1.set_xlabel("Current Heading / Stimulus Angle (°)", fontsize=12)
ax1.set_ylabel("Neuron Preferred Direction (PD) (°)", fontsize=12)
...
def plot_model_full(I_baseline, A_fast, A_phasic, tau_decay, J1, J0, title_suffix=""):
    ...
    time_plot = t_rel + T_STIM  # absolute time: 0 = recording start
...
# %%
# Load in vivo data — same pipeline as Alternating_soundsource.py
...
# %%
# Compute per-cell PSTHs — identical to Alternating_soundsource.py
...
# Sort speakers by angular distance to each cell's PD; column 0 = closest
...
# Training target: mean PSTH of each cell's closest speaker (bins_plot = ms relative to stim onset)
mean_vivo_rate = new_rate_sort[:, :, 0].mean(axis=0)

# %% [markdown]
# ## Temporal Trace Optimization to In Vivo Data
#
# Fit model parameters so the PD cell at 0° matches the smoothed in vivo average response.
#
...
# **Alignment:** model time relative to stim onset; in vivo bins shifted by `t_delay` (neural latency) to align.
...
def loss_temporal(params):
    ...
    # Extract the matched times and calculate raw squared errors
    ...
    # Default weight of 1.0 for the whole trace
    weights = np.ones_like(raw_errors) 
    
    # Identify the critical zone: The Spike and the Inhibitory Dip (e.g., 0 to 40ms)
    critical_zone = (matched_times >= 0) & (matched_times <= 40)
    
    # Make errors in this zone 20x more punishing to force the optimizer to hit the peaks
    weights[critical_zone] = 20.0 
