...
class HDRingAttractor:
    def __init__(self, N=180, J0=-0.6, J1=1.4, tau=10.0, dt=0.5,
        ...
        self.dt = float(dt)     # interpret as ms (so 0.5 = 0.5 ms)
        ...
        # --- Stim machinery (GLOBAL stim across all neurons) ---
        self.t_ms = 0.0                  # simulation time in ms
        self.stim_amp = 0.5              # amplitude of global stim pulse
    ...
    def stim_input(self):
        """
        Spatially uniform stimulus (entire ring).
        Active if current time is within ANY scheduled stim window.
        """
        ...
        # Keep events that haven't ended yet; check whether any event is active now
        keep = []
    ...
    def schedule_stim(self, delay_ms=9.0, width_ms=2.0):
        """
        Schedule a stim pulse that begins at t_now + delay_ms
        and lasts width_ms (so delay=9, width=2 gives 9–11 ms post-click).
        """
...
def run_interactive():
    ...
    s_ax_omega = fig.add_axes([0.10, 0.19, 0.60, 0.03])
    s_ax_cueang = fig.add_axes([0.10, 0.14, 0.60, 0.03])
    s_ax_cueamp = fig.add_axes([0.10, 0.09, 0.60, 0.03])
    s_ax_kappa = fig.add_axes([0.10, 0.04, 0.60, 0.03])

    # NEW: stim amplitude slider
    s_ax_stimamp = fig.add_axes([0.10, 0.235, 0.60, 0.03])
    ...
    # NEW: stim button
    b_ax_stim = fig.add_axes([0.75, 0.23, 0.22, 0.06])

    b_toggle = Button(b_ax_toggle, "Pause")
    b_reset = Button(b_ax_reset, "Reset")
    b_stim = Button(b_ax_stim, "Stim (9–11 ms)")
    ...
    def on_reset(_):
        ...
        # restore UI-controlled parameters
        model.cue_angle = s_cueang.val
        model.cue_amp = s_cueamp.val
        model.cue_kappa = s_kappa.val
        model.omega = s_omega.val
    ...
    def on_stim(_):
        # Delta-like pulse delivered 9–11 ms post-click
    ...
    s_stimamp.on_changed(on_slider)
    ...
    timer = fig.canvas.new_timer(interval=30)  # ms (UI refresh timer)
    ...
    def update(*_):
        ...
        txt.set_text(
            f"t = {model.t_ms:7.1f} ms\n"
            f"decoded HD: {hd:.3f} rad ({np.degrees(hd):.1f}°)\n"
            f"ω: {model.omega:.4f}  | cue: {cue_state} (A={model.cue_amp:.2f}, κ={
                model.cue_kappa:.1f})\n"
            f"stim amp (global): {model.stim_amp:.2f}  | pending events: {
                len(model._stim_events)}"
        )
