...
from spikeops import detect_peaks, extract_waveforms
import numpy as np
...
(remaining_spikes, waves) = extract_waveforms(
        filtered,
        pks,
        fs,
        0.0015,
...
plt.plot(np.mean(waves, axis=0))
plt.plot(np.mean(waves, axis=0)+np.std(waves, axis=0), '--')
plt.plot(np.mean(waves, axis=0)-np.std(waves, axis=0), '--')
plt.show()
